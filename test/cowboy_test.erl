...
gun_down(ConnPid) ->
  receive {gun_down, ConnPid, _, _, _} -> ok after 500 -> error(timeout) end.
...
raw_send({raw_client, Socket, Transport}, Data) -> Transport:send(Socket, Data).
...
raw_expect_recv({raw_client, _, _}, <<>>) -> ok;
