  ...
  case SessionCount > 20 of
    ...
    true ->
      ?LOG_WARNING("Connection limit exceeded", ?LOGGER_META),
      {
        stop,
        normal,
        ["421 ", Hostname, " is too busy to accept mail right now"]
      }
  end.
...
handle_EHLO(Hostname, Extensions, State) ->
  ...
  MyExtensions2 =
    case proplists:get_value(size, State#state.options) of
      undefined -> MyExtensions1;
      infinity -> [{"SIZE", "0"} | lists:keydelete("SIZE", 1, MyExtensions1)];

      Size when is_integer(Size), Size > 0 ->
        [
          {"SIZE", integer_to_list(Size)}
          | lists:keydelete("SIZE", 1, MyExtensions1)
        ]
    end,
...
handle_MAIL(<<"badguy@blacklist.com">>, State) -> {error, "552 go away", State};
...
handle_DATA(_From, _To, <<>>, State) -> {error, "552 Message too small", State};
      ...
      case proplists:get_value(parse, State#state.options, false) of
        ...
        true ->
          ...
          try mimemail:decode(Data) of
            _Result -> ?LOG_INFO("Message decoded successfully!", ?LOGGER_META)
          catch
            What:Why ->
              ?LOG_WARNING(
                "Message decode FAILED with ~p:~p",
                [What, Why],
                ?LOGGER_META
              ),
...
-spec handle_other(Verb :: binary(), Args :: binary(), #state{}) ->
  {string(), #state{}}.
...
handle_AUTH(_Type, _Username, _Password, _State) -> error.
...
queue_or_deliver(From, To, Data, Reference, State) ->
  ...
  case proplists:get_value(protocol, State#state.options, smtp) of
    ...
    lmtp ->
      ?LOG_INFO(
        "message from ~s delivered to ~p, body length ~p",
        [From, To, Length],
        ?LOGGER_META
      ),
