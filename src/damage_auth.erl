...
process_post(Params, Req, State) ->
    ...
    of
        ...
        _ ->
            {stop, cowboy_req:reply(400, #{}, <<"Bad Request.">>, Req), State}
    end.
...
emit_response(AuthResult, Req) ->
    ...
    case AuthResult of
        {error, Reason} ->
            {400, jsx:encode([{error, to_binary(Reason)}]), Req};
        {
            _User,
            {
                response,
...
binary_join([H | T], Sep) -> <<H/binary, Sep/binary, (binary_join(T, Sep))/binary>>;
binary_join([], _Sep) -> <<>>.
