...
state(Events, Config, Now) ->
    lists:foldl(
        fun(E, S) -> erm_lens_model:insert(E, Now, S) end,
        erm_lens_model:new(Config),
        Events
    ).
prepared() ->
    #{
...
with_req(Key, Value) ->
    P = prepared(),
    R = maps:get(request, P),
    P#{request := R#{Key => Value}}.
ui_state() ->
    #{
        config => #{},
        window => undefined,
...
config_bad_numeric_test() ->
    ?assertMatch(
        {error, {bad_lens_config, refresh_ms}}, erm_lens_config:normalize(#{refresh_ms => infinity})
    ).
...
config_bad_relay_test() ->
    ?assertMatch(
        {error, {bad_lens_config, relays}},
        erm_lens_config:normalize(#{relays => [<<"wss://u:p@example.com">>]})
    ).
config_bad_follow_keys_test() ->
    ?assertMatch(
        {error, {bad_lens_config, following}},
        erm_lens_config:normalize(#{following => [<<"not a key">>]})
    ).
...
improper_tag_is_rejected_test() ->
    E = photo(1, 2, 1000),
    ?assertEqual(
        {error, invalid_event}, erm_lens_nostr:validate(E#{<<"tags">> := [[<<"x">> | bad_tail]]})
    ).
...
large_timestamp_rejected_test() ->
    E = photo(1, 2, 1000),
    ?assertEqual(
        {error, invalid_event}, erm_lens_nostr:validate(E#{<<"created_at">> := (1 bsl 1000)})
    ).
...
byte_budget_test() ->
    Events = [
        (photo(N, 2, 1000))#{<<"content">> := binary:copy(<<"a">>, 60000)}
     || N <- lists:seq(1, 10)
    ],
...
prepared_atom_expiry_rejected_test() ->
    ?assertEqual(
        ...
        erm_lens_wallet:submit_tip(with_req(expires_at, infinity), #{network => <<"ae_uat">>})
    ).
prepared_negative_amount_rejected_test() ->
    ?assertEqual(
        {error, invalid_prepared_tip},
        erm_lens_wallet:validate_prepared(with_req(amount_base_units, -1))
    ).
prepared_float_amount_rejected_test() ->
    ?assertEqual(
        {error, invalid_prepared_tip},
        erm_lens_wallet:validate_prepared(with_req(amount_base_units, 1.5))
    ).
prepared_float_decimals_rejected_test() ->
    ?assertEqual(
        {error, invalid_prepared_tip}, erm_lens_wallet:validate_prepared(with_req(decimals, 1.5))
    ).
prepared_symbol_controls_rejected_test() ->
    ?assertEqual(
        {error, invalid_prepared_tip},
        erm_lens_wallet:validate_prepared(with_req(symbol, <<"TEST\nSEND">>))
    ).
prepared_missing_fields_rejected_test() ->
    ?assertEqual(
        {error, invalid_prepared_tip},
        erm_lens_wallet:validate_prepared(#{request => #{}, fee_aettos => 1})
    ).
submit_malformed_term_rejected_test() ->
    ?assertEqual(
        {error, invalid_prepared_tip},
        erm_lens_wallet:submit_tip(not_a_map, #{network => <<"ae_uat">>})
    ).
expired_preview_test() ->
    ?assertEqual(
        {error, tip_preview_expired},
        erm_lens_wallet:check_expiry(#{expires_at => erlang:system_time(second) - 1})
    ).
unbounded_future_preview_test() ->
    ?assertEqual(
        {error, tip_preview_expired},
        erm_lens_wallet:check_expiry(#{expires_at => erlang:system_time(second) + 10000})
    ).
invalid_adapter_configuration_test() ->
    ?assertEqual(
        {error, invalid_wallet_adapter},
        erm_lens_wallet:status(#{wallet_adapter => <<"not a module">>})
    ).

owned_job_result_test() ->
    {Pid, Mon} = erm_lens_worker:start(test_job, fun() -> {ok, 42} end),
    receive
        {test_job, Pid, Result} -> ?assertEqual({ok, 42}, Result)
    after 2000 -> error(job_result_timeout)
    end,
    erlang:demonitor(Mon, [flush]).
owned_job_exception_isolated_test() ->
    {Pid, Mon} = erm_lens_worker:start(test_job, fun() -> error(test_fault) end),
    receive
        {'DOWN', Mon, process, Pid, Reason} -> ?assertMatch({job_exit, _}, Reason)
    after 2000 -> error(job_exception_timeout)
    end.
...
owner_dies(Reason) ->
    ...
    Job =
        receive
            {job_started, J} -> J
        after 2000 -> error(no_job)
        end,
    Task =
        receive
            {task_started, T} -> T
        after 2000 -> error(no_task)
        end,
    ...
    try
        receive
            {'DOWN', TaskMon, process, Task, _} -> ok
        after 2000 -> error(orphan_task)
        end,
        receive
            {'DOWN', JobMon, process, Job, _} -> ok
        after 2000 -> error(orphan_guardian)
        end
    after
        exit(Task, kill),
        exit(Job, kill),
...
relay_silent_timeout_is_failure_test() ->
    ...
    ?assertError(
        {relay_query_timeout, 3, 1},
        erm_lens_relay:collect(self(), make_ref(), <<"sub">>, [], 4, End, 3, 1)
    ).
relay_cap_is_distinct_test() ->
    ?assertEqual(
        {3, 1, cap}, erm_lens_relay:collect(self(), make_ref(), <<"sub">>, [], 0, 0, 3, 1)
    ).
relay_oversized_ack_rejected_test() ->
    ...
    ?assertEqual(
        ...
        erm_lens_relay:await_ok(Pid, Stream, pub(1), erlang:monotonic_time(millisecond) + 1000)
    ).
relay_connection_wide_error_test() ->
    ...
    ?assertEqual(
        {error, {relay_error, closed}},
        erm_lens_relay:await_ok(Pid, Stream, pub(1), erlang:monotonic_time(millisecond) + 1000)
    ).
relay_binary_ack_rejected_test() ->
    Pid = self(),
    Stream = make_ref(),
    self() ! {gun_ws, Pid, Stream, {binary, <<1>>}},
    ?assertEqual(
...
connecting_retry_is_zero_test() ->
    S = #{
        status => #{
            url => #{
                state => connecting, next_retry_mono => erlang:monotonic_time(millisecond) + 5000
            }
        }
    },
    ?assertEqual(0, maps:get(retry_in_ms, maps:get(url, erm_lens_sync:status_snapshot(S)))).
...
decoder_path_escape_rejected_test() ->
    ?assertEqual(
        {error, unsafe_decoder_path},
        erm_lens_media:validate_result(
            {ok, <<"/etc/passwd">>}, #{url => <<"https://example.com/a.png">>}, #{
                cache_dir => "/tmp/lens-test"
            }
        )
    ).
decoder_expired_deadline_test() ->
    ?assertEqual(
        ...
        erm_lens_media:read_port(make_ref(), <<>>, erlang:monotonic_time(millisecond) - 1)
    ).
...
relay_log_label_redacts_query_test() ->
    Label = erm_lens_diagnostics:relay_label(
        <<"wss://example.com/private/path?token=supersecret">>
    ),
...
ui_busy_confirmation_preserves_intent_test() ->
    P = prepared(),
    Id = maps:get(request_id, maps:get(request, P)),
