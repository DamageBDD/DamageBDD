    ...
    try
        ...
        lists:foreach(
            ...
            [
                ...
                {refresh_ms, 30000, 3600000},
                {max_events, 1, 20000},
                {max_store_bytes, 262144, 268435456},
                {page_size, 1, 12},
                {relay_backoff_ms, 1000, 3600000},
        ...
        lists:foreach(
            ...
            [enabled, show_on_start, load_images, allow_mainnet]
        ),
        require(
            maps:get(relay_backoff_max_ms, C) >= maps:get(relay_backoff_ms, C),
        ...
        lists:foreach(
            fun(K) ->
                Keys = maps:get(K, C, []),
                require(
                    is_list(Keys) andalso length(Keys) =< 4096 andalso
                        lists:all(fun(P) -> erm_lens_nostr:is_hex(P, 64) end, Keys),
                    K
                )
            end,
            [muted, following]
        ...
        require(
            is_list(Hosts) andalso length(Hosts) =< 128 andalso
                lists:all(fun host_string/1, Hosts),
            media_hosts
        ),
        lists:foreach(
            fun(K) -> require(path_string(maps:get(K, C)), K) end,
