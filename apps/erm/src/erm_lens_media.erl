...
cancel_before(ReplyTo, Generation) ->
    gen_server:cast(?MODULE, {cancel_before, ReplyTo, Generation}).
...
handle_cast({request, Key, Image, To}, S) when is_pid(To), node(To) =:= node() ->
    ...
    case valid_image(Image) of
        false ->
            To ! {lens_media, Key, {error, invalid_image_request}},
            {noreply, S};
        true ->
            case {duplicate(Key, To, S), queue:len(Q) >= 48} of
                {true, _} ->
                    {noreply, S};
                {false, true} ->
                    To ! {lens_media, Key, {error, media_queue_full}},
                    {noreply, S};
                {false, false} ->
...
handle_cast({cancel_before, To, Gen}, S) when is_pid(To), is_integer(Gen) ->
    ...
    Keep = [
        Item
     || Item = {Key, _, Owner} <- queue:to_list(maps:get(queue, S)),
...
handle_info({media_timeout, Pid}, S) ->
    case maps:is_key(Pid, maps:get(active, S)) of
        true ->
            exit(Pid, kill),
            complete(Pid, {error, decoder_timeout}, S);
        false ->
            {noreply, S}
    end;
...
duplicate(Key, To, S) ->
    lists:any(
        fun(J) -> maps:get(key, J) =:= Key andalso maps:get(to, J) =:= To end,
        maps:values(maps:get(active, S))
    ) orelse
        lists:any(
            fun({K, _, P}) -> K =:= Key andalso P =:= To end,
...
decode(Image, C) ->
    try
        ...
        Args =
            [
                text(maps:get(media_script, C)),
                text(maps:get(url, Image)),
                "--cache",
                text(maps:get(cache_dir, C)),
                "--sha256",
                text(maps:get(sha256, Image, <<>>))
            ] ++ Hosts,
    ...
    case Left =< 0 of
        ...
        false ->
            receive
                ...
                {Port, {data, _}} ->
                    {error, oversized_decoder_response};
...
decoder_reply(Status, Data) ->
    case {Status, erm_lens_codec:decode(Data)} of
        ...
        _ ->
            {error, invalid_decoder_response}
    end.
...
validate_result({ok, File}, Image, C) ->
    ...
    Reported = text(File),
    case
        filename:pathtype(Reported) =:= absolute andalso
            filename:basename(Reported) =:= filename:basename(Expected)
    of
        false ->
        ...
        true ->
            ...
            case
                {
                    file:read_link_info(Root),
                    file:read_link_info(Expected),
                    file:read_link_info(Reported)
                }
            of
                {
                ...
                _ ->
                    {error, unsafe_decoder_file}
            end
    end;
validate_result(Error, _, _) ->
    Error.
...
png_header(Path) ->
    case file:open(Path, [read, binary, raw]) of
        {ok, Fd} ->
            try
                file:read(Fd, 8) =:= {ok, <<137, 80, 78, 71, 13, 10, 26, 10>>}
            after
                file:close(Fd)
            end;
        _ ->
