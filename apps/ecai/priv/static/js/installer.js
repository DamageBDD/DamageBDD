(function () {
  ...
  let detectedOsEl, dlAuto, dlAutoNote;
  ...
  const VARIANTS = {
    linux:   [{ value: 'bash',   label: 'Bash / systemd' }],
...
function buildWindowsBat({ host, domain, port, branch }) {
  ...
  function initInstallForm () {
    ...
    const os = detectOS();
    if (detectedOsEl) detectedOsEl.textContent = `Detected platform: ${os[0].toUpperCase()}${os.slice(1)}.`;
