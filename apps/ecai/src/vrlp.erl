...
-type decoded_data() :: binary() | integer() | atom() | tuple() | [decoded_data()].

-spec encode(Data) -> RLP when
    Data :: decoded_data(),
    RLP :: binary().
%% @doc
%% encode some data
...
encode(Atom) when is_atom(Atom) ->
    %% Encode atoms as their UTF-8 name bytes
    encode_binary(atom_to_binary(Atom, utf8));
encode(Tuple) when is_tuple(Tuple) ->
    %% Encode tuples as RLP lists of their elements
    encode_list(tuple_to_list(Tuple));
encode(List) when is_list(List) ->
    encode_list(List).

-spec encode_binary(Bytes) -> RLP when
    Bytes :: binary(),
    RLP :: binary().
%% @private
...
encode_binary(Bytes) when 55 < byte_size(Bytes), byte_size(Bytes) < (1 bsl 64) ->
    ...
    %% 183 = 128 + 55
    %% SizeOfSizeInt > 0
    <<(183 + SizeOfSizeInt), SizeBytes/binary, Bytes/binary>>.

-spec encode_list(List) -> RLP when
    List :: [decoded_data()],
    RLP :: binary().
%% @private
...
encode_list(List) ->
    ...
    if
        ...
        55 < Payload_Size ->
            ...
            %% 247 = 192 + 55
            %% SizeOfSizeInt > 0
            <<(247 + SizeOfSizeInt), SizeBytes/binary, Payload/binary>>
    end.
%% ------------------------------------------------------------------
%% Integer encoding (RLP)
%% ------------------------------------------------------------------

%% RLP integers are encoded as their minimal big-endian binary form.
%% Zero is encoded as the empty binary.
encode_integer(0) ->
    encode_binary(<<>>);
encode_integer(Int) ->
...
-spec decode(RLP) -> {Data, Rest} when
    RLP :: binary(),
    Data :: decoded_data(),
    Rest :: binary().
%% @doc
...
decode(<<Byte, Rest/binary>>) when Byte =< 183 ->
    ...
    <<Payload:PayloadByteLength/binary, Rest2/binary>> = Rest,
    {Payload, Rest2};
...
decode(<<Byte, Rest/binary>>) when Byte =< 191 ->
    ...
    <<ByteLengthInt:BitLengthOfByteLength, Rest2/binary>> = Rest,
    <<Payload:ByteLengthInt/binary, Rest3/binary>> = Rest2,
    {Payload, Rest3};
...
decode(<<Byte, Rest/binary>>) when Byte =< 247 ->
    ...
    <<ListPayload:ByteLengthOfListPayload/binary, Rest2/binary>> = Rest,
...
decode(<<Byte, Rest/binary>>) ->
    ...
    <<ByteLengthOfListPayload_int:BitLengthOfByteLengthOfListPayload_int, Rest2/binary>> = Rest,
    <<ListPayload_bytes:ByteLengthOfListPayload_int/binary, Rest3/binary>> = Rest2,
