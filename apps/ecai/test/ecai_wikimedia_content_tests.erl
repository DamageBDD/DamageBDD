...
exact_top_n_deduplicates_ranks_test() ->
    with_tmp(fun(Dir) ->
        ...
        Selector = #{
            selection_path => filename:join(Dir, "selection.jsonl"),
            limit => 2,
            candidate_limit => 3
        },
...
normalized(Id, Rank, Name) ->
    #{
        ...
        <<"visibility">> => #{
            <<"rank">> => Rank, <<"pageviews">> => 1000 - Rank, <<"active_months">> => 12
        }
    }.
...
with_tmp(Fun) ->
    Dir = filename:join(
        temp_dir(),
        "ecai-wikimedia-content-" ++ integer_to_list(erlang:unique_integer([positive, monotonic]))
    ),
    ...
    case file:read_link_info(Path) of
        {ok, Info} when element(3, Info) =:= directory ->
            case file:list_dir(Path) of
                {ok, Ns} -> lists:foreach(fun(N) -> remove_tree(filename:join(Path, N)) end, Ns);
                _ -> ok
            end,
            _ = file:del_dir(Path),
            ok;
