...
ensure_admin(_) -> {error, not_admin}.
...
re_ok(Text, Rx) ->
    case re:run(Text, Rx) of {match, _} -> true; _ -> false end.

%% ---------- Context / sudo ----------
step(_Cfg, Ctx, <<"Given">>, _N, ["I use sudo is", "true"], _)  -> maps:put(sudo, true, Ctx);
step(_Cfg, Ctx, <<"Given">>, _N, ["I use sudo is", "false"], _) -> maps:put(sudo, false, Ctx);
...
step(_Cfg, Ctx, <<"Then">>, _N,
     ["iptables must have a rule matching", Regex, "in chain", Chain], _) ->
    case run(Ctx, "iptables -S " ++ Chain) of
        {ok, #{stdout := Out}} ->
        ...
        {error, E} -> set_fail(Ctx, "iptables -S failed (~p)", [E])
    end;
...
step(_Cfg, Ctx0, <<"When">>, _N, ["I append iptables rule", Rule, "to chain", Chain], _) ->
    case ensure_admin(Ctx0) of
        ...
        {error, _} -> set_fail(Ctx0, "Admin privileges required", [])
    end;
...
step(_Cfg, Ctx, <<"Then">>, _N, ["nftables list must contain", Regex], _) ->
    case run(Ctx, "nft list ruleset") of
        ...
        {error, E} -> set_fail(Ctx, "nft list ruleset failed (~p)", [E])
    end;
...
step(_Cfg, Ctx0, <<"When">>, _N, ["I add nft rule", Rule, "to table", Table, "chain", Chain], _) ->
    case ensure_admin(Ctx0) of
        ok ->
            ...
            case run(Ctx0, Cmd) of
                {ok, _}    -> Ctx0;
                {error, E} -> set_fail(Ctx0, "Failed to add nft rule to ~p/~p: ~p", [Table, Chain, E])
            end;
        {error, _} -> set_fail(Ctx0, "Admin privileges required", [])
    end;

step(_Cfg, Ctx0, <<"When">>, _N, ["I delete nft rule handle", Handle, "from table", Table, "chain", Chain], _) ->
    case ensure_admin(Ctx0) of
        ok ->
            Cmd = sudo_prefix(Ctx0) ++ "nft delete rule " ++ Table ++ " " ++ Chain ++ " handle " ++ Handle,
            case run(Ctx0, Cmd) of
                {ok, _}    -> Ctx0;
...
step(_Cfg, Ctx, <<"Then">>, _N, ["sysctl key", Key, "must be", Val], _) ->
    case run(Ctx, "sysctl -n " ++ Key) of
        ...
        {error, E} -> set_fail(Ctx, "sysctl -n ~p failed (~p)", [Key, E])
    end;
...
step(_Cfg, Ctx, <<"Then">>, _N, ["SSH must disallow password auth"], _) ->
    case run(Ctx, "sshd -T") of
        ...
        {error, E} -> set_fail(Ctx, "sshd -T failed (~p)", [E])
    end;
...
step(_Cfg, Ctx, <<"Then">>, _N, ["port", Port, "must be", "open", "tcp"], _) ->
    case run(Ctx, "ss -lnt") of
        ...
        {error, E} -> set_fail(Ctx, "ss -lnt failed (~p)", [E])
    end;
