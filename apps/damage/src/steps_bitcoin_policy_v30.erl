...
step(_Config, Context, <<"Then">>, _N, ?THEN_MEMPOOL_SHOULD_BE, _Raw) ->
    case Verdict of
        <<"accepted">> -> ensure_allowed(Context, true),  Context;
        <<"rejected">> -> ensure_allowed(Context, false), Context;
        _              -> error({unsupported_verdict, Verdict})
    end;
