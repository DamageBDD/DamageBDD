...
cmp_parts([{i, _} | _] = A, [{s, _} | _] = B) ->
    cmp_parts(A, B);
cmp_parts([{s, _} | _] = A, [{i, _} | _] = B) ->
    cmp_parts(A, B).
...
ensure_choco(Context) ->
    case ensure_windows(Context) of
        ...
        {ok, _} ->
            Res = ps(
                "Get-Command choco -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Name"
            ),
...
step(_Config, Context, <<"When">>, _N, ["I choco install", Pkg], _) ->
    case is_allowed(Pkg, Context) of
        ...
        false ->
            put_fail(
                Context,
                "Package ~p is not whitelisted. Add it via 'I allow chocolatey packages'.",
                [Pkg]
            )
    end;
step(_Config, Context, <<"When">>, _N, ["I choco upgrade", Pkg], _) ->
    case is_allowed(Pkg, Context) of
        true ->
            ...
            C1;
        false ->
            put_fail(Context, "Package ~p is not whitelisted.", [Pkg])
    end;
...
step(_Config, Context, <<"Then">>, _N, ["the choco package", Pkg, "should be installed"], _) ->
    case
        run_choco(
            Context,
            io_lib:format(
                "list --local-only --exact ~s --limit-output", [binary_to_list(unquote(Pkg))]
            )
        )
    of
        {error, Ctx} ->
