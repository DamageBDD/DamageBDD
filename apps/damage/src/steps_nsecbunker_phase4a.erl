%%--------------------------------------------------------------------
%% DamageBDD steps for Phase 4A dev DamageBDD key rehearsal.
%%
%% These steps exercise the dev/disposable key ceremony script and verify
%% that the generated report contains public identity material only.
%%--------------------------------------------------------------------
-module(steps_nsecbunker_phase4a).
...
-define(NS, nsecbunker_phase4a).

-define(S_SCRIPT_EXISTS, ["the Phase 4A dev key ceremony script exists"]).
-define(S_VAULT_CONFIGURED, ["the Phase 4A dev vault path is configured"]).
-define(S_RUN_CEREMONY, ["I run the Phase 4A dev key ceremony"]).
-define(S_REPORT_EXISTS, ["the Phase 4A dev key report MUST exist"]).
-define(S_REPORT_HEX_PUBKEY, [
    "the Phase 4A dev key report MUST contain a", "64", "lowercase hex pubkey"
]).
-define(S_REPORT_HEX_PUBKEY_FLAT, [
    "the Phase 4A dev key report MUST contain a 64 lowercase hex pubkey"
]).
-define(S_REPORT_NPUB, ["the Phase 4A dev key report MUST contain an npub"]).
-define(S_REPORT_NO_SECRET, ["the Phase 4A dev key report MUST NOT contain secret material"]).
...
step(_Config, Context, _Keyword, _Line, ?S_VAULT_CONFIGURED, _Args) ->
    ...
    put_ns(Context, (ns(Context))#{
        ...
        json_report => unicode:characters_to_binary(
            filename:join(ReportDir, "PHASE4A_DEV_DAMAGEBDD_KEY.json")
        ),
        md_report => unicode:characters_to_binary(
            filename:join(ReportDir, "PHASE4A_DEV_DAMAGEBDD_KEY.md")
        )
    });
...
step(_Config, Context0, _Keyword, _Line, ?S_REPORT_EXISTS, _Args) ->
    ...
    case filelib:is_regular(Json) andalso filelib:is_regular(Md) of
        true -> Context;
        false -> error({phase4a_dev_key_report_missing, #{json => Json, md => Md}})
    end;
step(_Config, Context0, _Keyword, _Line, ?S_REPORT_HEX_PUBKEY, _Args) ->
    assert_report_pubkey(ensure_phase4a_defaults(Context0));
...
step(_Config, Context0, _Keyword, _Line, ?S_REPORT_NPUB, _Args) ->
    ...
    case is_binary(Npub) andalso is_npub(Npub) of
        true -> Context;
        false -> error({phase4a_invalid_npub, Npub})
    end;
step(_Config, Context0, _Keyword, _Line, ?S_REPORT_NO_SECRET, _Args) ->
    ...
    case secret_leak(JsonMap) of
        false -> ok;
        Leak -> error({phase4a_json_report_secret_material_leaked, Leak})
    end,
    case secret_value(Md) of
        false -> Context;
        true -> error({phase4a_markdown_report_secret_material_leaked, <<"[REDACTED]">>})
    end.
...
assert_report_pubkey(Context) ->
    ...
    case is_binary(Pubkey) andalso is_lower_hex_64(Pubkey) of
        true -> Context;
        false -> error({phase4a_invalid_pubkey_hex, Pubkey})
    end.

ensure_phase4a_defaults(Context0) ->
    Context1 =
...
read_json_report(Context) ->
    ...
    try jsx:decode(Bin, [return_maps]) of
        ...
        Other -> error({phase4a_json_report_not_object, Other})
    catch
        Class:Reason -> error({phase4a_json_report_decode_failed, Class, Reason, Path})
    end.
...
read_file(Path) when is_list(Path) ->
    case file:read_file(Path) of
        {ok, Bin} -> Bin;
        {error, Reason} -> error({phase4a_report_read_failed, Path, Reason})
    end.

get_report_field(BinKey, AtomKey, Map) when is_map(Map) ->
    case maps:get(BinKey, Map, maps:get(AtomKey, Map, undefined)) of
        undefined -> error({phase4a_report_missing_field, BinKey, Map});
