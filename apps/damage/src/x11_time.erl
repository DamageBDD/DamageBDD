...
-record(state, {
    current = undefined,      % #{class:=binary(), title:=binary(), winid:=binary()}
    last_ts = 0,              % monotonic time (second)
    by_class = #{},           % #{<<"Emacs">> => Seconds, ...}
    by_title = #{},           % #{<<"Emacs — foo.erl">> => Seconds, ...}
    aliases = #{}             % #{<<"code">> => [<<"Code">>, <<"codium">>], ...}
}).
...
maybe_roll_time(S=#state{current=#{class := C, title := T}, last_ts=Last, by_class=BC0, by_title=BT0}) ->
    Now = now(),
    Delta = max(0, Now - Last),
    BC = maps:update_with(C, fun(V)->V+Delta end, Delta, BC0),
    BT = maps:update_with(T, fun(V)->V+Delta end, Delta, BT0),
    S#state{by_class=BC, by_title=BT, last_ts=Now}.
