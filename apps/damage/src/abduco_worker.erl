...
-record(state, {
    name,
    cmd,                         %% string or argv list (we pass through to erlexec)
    env = #{},                   %% #{ "K" => "V" }
    exec_pid = undefined,        %% Erlang pid returned by erlexec (linked/monitored)
    os_pid   = undefined         %% integer OS PID of the child
}).
...
handle_call(status, _From, S = #state{name=Name, cmd=Cmd, env=Env, exec_pid=EP, os_pid=OP}) ->
...
handle_call({signal, Sig0}, _From, S = #state{exec_pid=EP, os_pid=OP}) ->
    ...
    Reply =
        case {is_pid_alive(EP), is_os_alive(OP)} of
            {true, true}  -> catch exec:kill(EP, Sig), ok;
            {true, false} -> %% child may have reaped; try via ExecPid anyway
                             catch exec:kill(EP, Sig), ok;
...
interpolate_env(Map) when is_map(Map) ->
    maps:from_list([{to_list(K), secrets:interpolate_template(to_list(V))} || {K,V} <- maps:to_list(Map)]).
