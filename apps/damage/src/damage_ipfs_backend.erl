...
execute({add, What}, C) -> with_connection(fun(P, T) -> ipfs:add(P, What, T) end, C);
execute({get, Cid0, Path}, C) ->
...
execute({pin, Hashes}, C) -> with_connection(fun(P, T) -> ipfs:pin(P, Hashes, T) end, C);
execute({ensure_pin, Cid0}, C) ->
    with_cid(Cid0, fun(Cid) ->
        rpc("pin/add", [{"arg", Cid}, {"recursive", "true"}], C)
    end);
execute({unpin, Cid0}, C) ->
    with_cid(Cid0, fun(Cid) ->
        case rpc("pin/rm", [{"arg", Cid}, {"recursive", "true"}], C) of
            {error, {http_error, 500, Body}} = E ->
                case not_pinned(Body) of true -> {ok, already_unpinned}; false -> E end;
            R -> R
        end
    end);
execute({pin_check, Cid0}, C) ->
    with_cid(Cid0, fun(Cid) ->
        case rpc("pin/ls", [{"arg", Cid}, {"type", "recursive"}], C) of
            ...
            {error, {http_error, 500, Body}} = E ->
                case not_pinned(Body) of true -> {ok, false}; false -> E end;
            {ok, _} -> {error, invalid_pin_response};
            E -> E
        end
    end);
execute({explicit_pin_check, Cid0}, C) ->
    with_cid(Cid0, fun(Cid) ->
...
execute(version, C) -> rpc("version", [], C);
execute(identity, C) -> rpc("id", [], C);
execute(swarm_peers, C) -> rpc("swarm/peers", [], C);
    ...
    case rpc("swarm/connect", [{"arg", Addr}], C) of
        {error, {http_error, 500, Body}} = E ->
            ...
            case binary:match(Lower, <<"already connected">>) of
                nomatch -> E; _ -> {ok, already_connected}
            end;
        R -> R
    end;
execute(_, _) -> {error, unsupported_operation}.

%% The ipfs dependency builds Kubo query strings with cow_qs, which expects
%% query values to be binaries. DamageBDD historically accepts both Erlang
%% strings and binaries for CIDs, so canonicalize once at this backend boundary
%% before calling ipfs:get/cat/ls or the control RPC helpers.
with_cid(Cid0, Fun) when is_function(Fun, 1) ->
                ...
                {ok, _} ->
                    ...
                    case ipfs:start_link(Server) of
                        {ok, Pid} when is_pid(Pid) ->
                            try Fun(Pid, maps:get(request_timeout_ms, C))
                            after
                                ...
                                catch exit:_ -> exit(Pid, kill) end
                            end;
                        {error, _} = E -> E;
                        _ -> {error, invalid_start_response}
                    end;
                {error, _} -> {error, gun_unavailable}
            end;
        _ -> {error, legacy_ipfs_transport_requires_plain_http}
    end.
...
rpc_started(Command, Args, C) ->
    ...
    Url = Base ++ "/api/v0/" ++ Command ++ case Query of [] -> ""; _ -> "?" ++ Query end,
    HttpOpts0 = [{timeout, maps:get(http_timeout_ms, C)},
                 {connect_timeout, maps:get(connect_timeout_ms, C)}, {autoredirect, false}],
    case tls_options(Base) of
        {error, _} = Error -> Error;
        {ok, TLS} ->
            case httpc:request(post, {Url, maps:get(headers, C),
                                      "application/x-www-form-urlencoded", <<>>},
                               TLS ++ HttpOpts0, [{body_format, binary}]) of
                {ok, {{_, Code, _}, _, Body}} ->
...
tls_options("https://" ++ _) ->
    case application:ensure_all_started(ssl) of
        {ok, _} ->
            try {ok, [{ssl, [{verify, verify_peer}, {cacerts, public_key:cacerts_get()},
                             {customize_hostname_check,
                              [{match_fun, public_key:pkix_verify_hostname_match_fun(https)}]}]}]}
            catch _:_ -> {error, tls_ca_unavailable} end;
...
decode_json(Bin) ->
    try
        Value = case code:ensure_loaded(json) of
            {module, json} -> json:decode(Bin);
            _ -> case code:ensure_loaded(jsx) of
                {module, jsx} -> jsx:decode(Bin, [return_maps]);
                _ -> jiffy:decode(Bin, [return_maps])
            end
        end,
        {ok, Value}
    catch error:undef -> {error, json_decoder_unavailable};
...
limit_result({ok, B}, C) when is_binary(B) ->
    case byte_size(B) =< maps:get(max_response_bytes, C) of
        true -> {ok, B}; false -> {error, response_too_large}
    end;
limit_result(B, C) when is_binary(B) ->
    case limit_result({ok, B}, C) of {ok, B} -> B; E -> E end;
limit_result(R, _) -> R.
