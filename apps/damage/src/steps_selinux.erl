...
step(Config, Ctx, K, N, Args, Extra) ->
    try do_step(Config, Ctx, K, N, Args, Extra)
    catch C:E:St ->
...
do_step(_Config, Ctx, <<"Then">>, _N, [<<"selinux status must be">>, Expected], _Extra) ->
    StatusRaw =
        case maps:get(selinux_status_raw, Ctx, undefined) of
            undefined ->
                case run_bin("getenforce", []) of
                    {ok, Out, Err} -> put_last(Ctx, ["getenforce"], Out, Err), Out;
                    _ -> <<"">>
                end;
            V -> V
        end,
...
do_step(_Config, Ctx, <<"Then">>, _N,
       [<<"processes of user">>, User, <<"must be in selinux domain containing">>, Token], _Extra) ->
    ...
    case Matches of
        [] ->
            maps:put(fail,
                io_lib:format("No processes for user ~s in domain containing ~s", [User, Token]), Ctx);
        _  ->
            maps:put(selinux_user_domain_matches, Matches, Ctx)
    end;

%% When I write a selinux policy template for "name" to "/tmp/name.te"
do_step(_Config, Ctx, <<"When">>, _N, [<<"I write a selinux policy template for">>, Name, <<"to">>, Path], _Extra) ->
...
do_step(_Config, Ctx, <<"Then">>, _N, [<<"user">>, User, <<"must have a process in domain">>, ExactDom], _Extra) ->
    Procs = ensure_procs(Ctx),
    ...
    case HasAny of
        true  -> Ctx;
        false -> maps:put(fail, io_lib:format("No process of user ~s in domain ~s", [User, ExactDom]), Ctx)
    end;
...
do_step(_Config, Ctx, <<"Then">>, _N, [<<"the last selinux stdout must contain">>, Sub], _Extra) ->
    case maps:get(selinux_last_run, Ctx, undefined) of
        ...
        _ -> maps:put(fail, <<"No previous SELinux command captured">>, Ctx)
    end;
...
take_while(Pred, [H|T]) ->
    case Pred(H) of true -> [H|take_while(Pred, T)]; false -> [] end;
take_while(_Pred, []) -> [].

is_digit($0)->true; is_digit($1)->true; is_digit($2)->true; is_digit($3)->true;
is_digit($4)->true; is_digit($5)->true; is_digit($6)->true; is_digit($7)->true;
is_digit($8)->true; is_digit($9)->true; is_digit(_) -> false.

is_word($_)->true; is_word($-)->true; is_word($.)->true;
...
sanitize_name(N) ->
    ...
    Safe = [case is_word(C) of true -> C; false -> $_ end || C <- L],
    lists:concat(Safe).
