...
ok_or_fail(Context, {ok, _} = _Res, _Msg) ->
    Context;
...
step(_Cfg, Context0, <<"When">>, _N, ["I install core gaming packages"], _Meta) ->
    ...
    case maps:is_key(fail, Context1) of
        ...
        false ->
            ...
            case maps:is_key(fail, Context2) of
                true ->
                    Context2;
                false ->
                    ok_or_fail(
...
step(_Cfg, Context0, <<"When">>, _N, ["I enable gaming services"], _Meta) ->
    ...
    case maps:is_key(fail, Context1) of
        ...
        false ->
            ok_or_fail(
                Context1,
                run(Context1, "sudo systemctl enable --now nvidia-persistenced.service"),
...
step(_Cfg, Context0, <<"When">>, _N, ["I set CPU governor to", "performance"], _Meta) ->
    ...
    case maps:is_key(fail, Context1) of
        ...
        false ->
            ok_or_fail(
                Context1,
                run(Context1, "sudo cpupower frequency-set -g performance"),
                "set governor"
            )
    end;
%% NVIDIA power & clocks ---------------------------------------------
            ...
            case OutTrim =:= WantTrim of
                ...
                false ->
                    maps:put(
                        fail,
                        io_lib:format(
                            "PowerMizer mismatch: want ~s got ~s",
                            [WantTrim, OutTrim]
                        ),
                        Context0
                    )
...
step(_Cfg, Context0, <<"When">>, _N, ["I disable the compositor while gaming (KDE)"], _Meta) ->
    ...
    case maps:is_key(fail, Context1) of
        ...
        false ->
            ok_or_fail(
                Context1, run(Context1, "qdbus org.kde.KWin /KWin reconfigure"), "kwin reload"
            )
    end;
%% User limits bump ---------------------------------------------------
        ...
        {ok, #{stdout := Out}} ->
            case string:trim(Out) of
                ...
                Other ->
                    maps:put(
                        fail,
                        io_lib:format("service ~s not active (~s)", [Service, Other]),
                        Context0
                    )
            end;
        Err ->
            ok_or_fail(Context0, Err, "systemctl is-active")
    end.
