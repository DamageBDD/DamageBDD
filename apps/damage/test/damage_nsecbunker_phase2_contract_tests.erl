...
authorized_ping_allowed_test() ->
    setup(),
    ?assertMatch({ok, _}, damage_nsecbunker_policy:authorize(request(<<"ping">>), policy(), ?NOW, vault_pubkey())).

unknown_client_rejected_test() ->
    setup(),
    Req = (request(<<"get_public_key">>))#{requester_pubkey => <<"UNKNOWN_CLIENT_PUBKEY_HEX">>},
    ?assertEqual({error, client_not_authorized}, damage_nsecbunker_policy:authorize(Req, policy(), ?NOW, vault_pubkey())).

unsupported_method_rejected_test() ->
    setup(),
    Req = request(<<"publish_event">>),
    ?assertEqual({error, method_not_allowed}, damage_nsecbunker_policy:authorize(Req, policy(), ?NOW, vault_pubkey())).

kind1_signing_allowed_by_policy_test() ->
    setup(),
    ?assertMatch({ok, _}, damage_nsecbunker_policy:authorize(sign_request(valid_event(1)), policy(), ?NOW, vault_pubkey())).

kind30023_signing_allowed_by_policy_test() ->
    setup(),
    ?assertMatch({ok, _}, damage_nsecbunker_policy:authorize(sign_request(valid_event(30023)), policy(), ?NOW, vault_pubkey())).

unsupported_kind_rejected_test() ->
    setup(),
    Event = #{kind => 4, created_at => ?NOW, tags => [], content => <<"no">>},
    ?assertEqual({error, kind_not_allowed}, damage_nsecbunker_policy:authorize(sign_request(Event), policy(), ?NOW, vault_pubkey())).

stale_request_rejected_test() ->
    setup(),
    Event = (valid_event(1))#{created_at => ?NOW - 1000},
    Req = (sign_request(Event))#{created_at => ?NOW - 1000},
    ?assertEqual({error, request_stale}, damage_nsecbunker_policy:authorize(Req, policy(), ?NOW, vault_pubkey())).

future_request_rejected_test() ->
    setup(),
    Event = (valid_event(1))#{created_at => ?NOW + 1000},
    Req = (sign_request(Event))#{created_at => ?NOW + 1000},
    ?assertEqual({error, request_from_future}, damage_nsecbunker_policy:authorize(Req, policy(), ?NOW, vault_pubkey())).

kind30023_missing_tags_rejected_test() ->
    setup(),
    Event = (valid_event(30023))#{tags => []},
    ?assertEqual({error, missing_required_tag}, damage_nsecbunker_policy:authorize(sign_request(Event), policy(), ?NOW, vault_pubkey())).

kind30023_active_content_rejected_test() ->
    setup(),
    Event = (valid_event(30023))#{content => <<"# Bad\n<script>alert(1)</script>">>},
    ?assertEqual({error, active_content_not_allowed}, damage_nsecbunker_policy:authorize(sign_request(Event), policy(), ?NOW, vault_pubkey())).

replay_duplicate_same_payload_is_idempotent_test() ->
    setup(),
    ?assertEqual(ok, damage_nsecbunker_replay:check_and_mark(<<"client">>, <<"REQ">>, <<"HASH-A">>)),
    ?assertEqual({ok, duplicate_same_payload}, damage_nsecbunker_replay:check_and_mark(<<"client">>, <<"REQ">>, <<"HASH-A">>)).

replay_conflict_rejected_test() ->
    setup(),
    ?assertEqual(ok, damage_nsecbunker_replay:check_and_mark(<<"client">>, <<"REQ">>, <<"HASH-A">>)),
    ?assertEqual({error, replay_conflict}, damage_nsecbunker_replay:check_and_mark(<<"client">>, <<"REQ">>, <<"HASH-B">>)).

rate_limited_client_rejected_test() ->
    setup(),
    ok = damage_nsecbunker_rate:seed(<<"client">>, ?NOW, 30),
    ?assertEqual({error, rate_limited}, damage_nsecbunker_rate:check_and_mark(<<"client">>, ?NOW, 30, 60)).

vault_integrity_failure_rejected_test() ->
    setup(),
    Vault = #{sealed => false, integrity => failed, pubkey_hex => vault_pubkey()},
    ?assertEqual({error, vault_integrity_failed}, damage_nsecbunker_vault_guard:assert_ready(Vault, vault_pubkey())).

vault_pubkey_mismatch_rejected_test() ->
    setup(),
    Vault = #{sealed => false, integrity => ok, pubkey_hex => <<"DIFFERENT">>},
    ?assertEqual({error, vault_pubkey_mismatch}, damage_nsecbunker_vault_guard:assert_ready(Vault, vault_pubkey())).
