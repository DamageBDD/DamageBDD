...
parse_transform_(Forms, Opts) ->
    ...
    Res = parse_trans:do_insert_forms(
        ...
        [
            ...
            {attribute, ?LINE, export, [{M, 1} || {M, _} <- AllMods]}
        ],
...
xform({function, _, groups, 0, _}, AllMods) ->
    {done,
        {function, ?LINE, groups, 0, [
            {clause, ?LINE, [], [], [
                erl_parse:abstract(
                    [{eunit, [M || {M, _Fs} <- AllMods]}], [{line, ?LINE}]
                )
            ]}
        ]}};
xform(F, _) ->
    ...
    case beam_lib:chunks(F, [imports, exports]) of
        {ok, {Mod, Chunks}} ->
            case
                lists:member(
                    {eunit, test, 1},
                    proplists:get_value(imports, Chunks, [])
                )
            of
                true ->
