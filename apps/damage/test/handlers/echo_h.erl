...
init(Req, Opts) ->
  case cowboy_req:binding(arg, Req) of
    undefined -> echo(cowboy_req:binding(key, Req), Req, Opts);
    Arg -> echo_arg(Arg, Req, Opts)
  end.
  ...
  {_, Body, Req} =
    case cowboy_req:path(Req0) of
      ...
      <<"/spawn", _/bits>> ->
        ...
        Pid =
          spawn_link(
            fun () -> Parent ! {self(), cowboy_req:read_body(Req0)} end
          ),
...
echo(<<"read_urlencoded_body">>, Req0, Opts) ->
  ...
  case {Path, Opts} of
    {<<"/opts", _/bits>>, #{crash := true}} ->
      ct_helper:ignore(cowboy_req, read_body, 2);

    {_, #{crash := true}} ->
      ct_helper:ignore(cowboy_req, read_urlencoded_body, 2);
...
echo(<<"read_and_match_urlencoded_body">>, Req0, Opts) ->
  ...
  {ok, Body, Req} =
    case Path of
      <<"/opts", _/bits>> ->
        cowboy_req:read_and_match_urlencoded_body([], Req0, Opts);
...
echo_arg(Arg0, Req, Opts) ->
  ...
  Value =
    case cowboy_req:binding(default, Req) of
...
value_to_iodata(V) when is_list(V); is_tuple(V); is_map(V) ->
  io_lib:format("~999999p", [V]);
