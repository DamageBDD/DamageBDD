...
erlc -DTEST +warnings_as_errors -o "$BUILD" \
    ...
    apps/damage/src/damage_release.erl \
    apps/damage/src/steps_hotcode.erl \
    apps/damage/test/damage_hotcode_tests.erl
...
erl -boot start_clean -noshell -noinput -pa "$BUILD" -eval '
    try
        ...
        [] = damage_hotcode_tests:hardening_test_(),
        ok = application:set_env(damage_hotcode_tests, isolated_node, true),
        Tests = {inorder, Cases} = damage_hotcode_tests:hardening_test_(),
        true = (is_list(Cases) andalso Cases =/= []),
        io:format("Running ~B isolated hot-code hardening cases.~n", [length(Cases)]),
        case eunit:test([damage_release, {"isolated hot-code hardening", Tests}], [verbose]) of
            ok -> halt(0);
            _ -> halt(1)
        end
    catch
